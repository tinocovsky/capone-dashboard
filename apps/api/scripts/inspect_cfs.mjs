...
let total = 0, withCfs = 0, sample = null;
for (let page = 1; page <= 3; page++) {
  ...
  for (const o of j.opportunities ?? []) {
    ...
    total++;
    if (o.customFields && o.customFields.length > 0) {
      withCfs++;
      if (!sample) sample = { name: o.name, createdAt: o.createdAt, cfs: o.customFields };
    }
  }
}
console.log("opps em vendas (sem filtro data):", total, "| com cfs:", withCfs);
if (sample) console.log("\nsample:", JSON.stringify(sample, null, 2).slice(0, 800));

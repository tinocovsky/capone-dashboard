...
// Pega 3 páginas, agrupa por assignedTo
const closers = {};
for (let page = 1; page <= 3; page++) {
  ...
  for (const o of j.opportunities ?? []) {
    ...
    if (o.assignedTo) closers[o.assignedTo] = (closers[o.assignedTo] ?? 0) + 1;
  }
}
console.log("=== closers (assignedTo) únicos em Vendas ===");
for (const [id, count] of Object.entries(closers).sort((a, b) => b[1] - a[1])) {
